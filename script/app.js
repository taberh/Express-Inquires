(function(doc, undefined) {
    ...
    var fullCompanyElem = doc.getElementById("fullCompany"), 
		...
		searchInputElem = doc.querySelector("#search input"), 
    ...
    HistoryData = {
		TPL: '<li code="{code}" name="{name}" ticket="{ticket}" title="单号：{ticket}"><span>{name}</span><b>{date}</b></li>',
		...
		add: function(code, ticket, name) {
            this.data = this.data || [];
            this.data.unshift({code: code,name: name,ticket: ticket,date: (new Date()).toJSON()});
            this.data.length > 4 ? this.data.splice(4) : void 0;
		...
		render: function(data) {
            ...
            if (Object.prototype.toString.call(data) === "[object Object]") {
                fragment = template(this.TPL, {code: data.code,name: data.name,date: formatDate(data.date),ticket: data.ticket})
            } else {
                data.forEach(function(obj, i) {
                    fragment += template(self.TPL, {code: obj.code,name: obj.name,date: formatDate(obj.date),ticket: obj.ticket})
                })
            }
            historyQueryElem.innerHTML = fragment + historyQueryElem.innerHTML;
            historyQueryElem.children.length > 4 ? historyQueryElem.removeChild(historyQueryElem.lastChild) : void 0;
            historyQueryElem.parentNode.style.display = "block";
            doc.body.style.width = "475px"
        }
	};
    ...
    function bind() {
        var moreElem = doc.querySelector("#more button"), 
			clearElem = doc.querySelector("#clear button"), 
        ...
        moreElem.addEventListener("click", function(e) {
            window.getComputedStyle(fullCompanyElem, null).display === "none" ? 
					(fullCompanyElem.style.display = "block") : 
    ...
    function submit(e) {
        ...
        queryData(code, ticket, function(result) {
            HistoryData.add(code, ticket, thumb.innerHTML)
        })
    };
